...
public class VirtualDevice {
    ...
    // Optional flush callback for internet-connected bridge devices.
    // When set, the bridge invokes this after storing a packet to forward it to the backend.
    private PacketFlushListener flushListener;
    ...
    public void receive(MeshPacket packet) {
        ...
        // Step 3: Bridge flush — internet-connected devices forward the stored packet to the backend.
        // This happens independently of TTL. Even a TTL=0 packet is flushed after storage.
        // The packet is passed as-is — no decryption or modification occurs here.
        if (hasInternet && flushListener != null) {
            flushListener.onPacketFlushed(packet);
        }

        // Step 4: TTL check — if TTL is zero or below, stop relaying.
        // The packet is stored but not forwarded to neighbors.
        if (packet.ttl() <= 0) {
            return;
        }

        // Step 5: Decrement TTL and relay to all neighbors.
    ...
    /**
     * Sets the flush listener for this device.
     * Only internet-connected bridge devices should use this.
     * When set, the bridge calls the listener after storing a packet,
     * forwarding the encrypted MeshPacket to the backend.
     *
     * @param listener the callback to invoke when a packet is flushed
     */
    public void setFlushListener(PacketFlushListener listener) {
